...
{
    [JsonInclude]
    public Dictionary<string, bool> Unlocked { get; private set; } = new();

    [JsonInclude]
    public Dictionary<string, int> ModeWins { get; private set; } = new();
        ...
        int maxConsecutiveReveals, BestTimes bestTimes)
    {
        ...
        // A10: 100 consecutive reveals without flagging
    ...
    {
        if (!ModeWins.ContainsKey(modeName))
            ModeWins[modeName] = 0;
        ModeWins[modeName]++;
    }
