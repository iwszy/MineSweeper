...
public class GameLogic
{
    ...
    public void RevealCell(int x, int y) {
        ...
        if (DisplayState[x, y] == CellState.Revealed) {
            return;
        }
        if (DisplayState[x, y] == CellState.Flagged) {
            return;
        }
        ...
        if (adjacent > 0) {
            ...
            while (queue.Count > 0) {
                ...
                if (!Model.IsInBounds(cx, cy)) {
                    continue;
                }
                if (DisplayState[cx, cy] == CellState.Revealed) {
                    continue;
                }
                if (DisplayState[cx, cy] == CellState.Flagged) {
                    continue;
                }
                if (Model.MineMap[cx, cy]) {
                    continue;
                }
